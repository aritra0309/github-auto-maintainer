...
from github_auto_maintainer.skills.pr_summary import PRSummarySkill, _compute_routing

FIXTURES = Path(__file__).resolve().parent.parent / "fixtures"
BASE = "https://api.github.com"


def _golden_pr_summary_json() -> str:
    return (FIXTURES / "pr_summary_golden.json").read_text()
...
def _make_event(payload: dict[str, object] | None = None) -> NormalizedEvent:
    ...
    return NormalizedEvent(
        ...
        delivery_id="test-delivery-pr-summary",
...
def test_does_not_handle_synchronize() -> None:
    skill = PRSummarySkill()
    event = NormalizedEvent(
...
def test_compute_routing_large() -> None:
    task_type, complexity = _compute_routing(total_changed=500, total_files=15)
    assert task_type == TaskType.SUMMARIZATION
...
async def test_execute_end_to_end() -> None:
    golden = _golden_pr_summary_json()
    ...
    assert result.skill_name == "pr_summary"
    assert result.event_delivery_id == "test-delivery-pr-summary"
    ...
    assert result.decision.key_changes == (
    ...
    assert isinstance(action, PRReviewSummaryAction)
    assert action.owner == "octocat"
    assert action.repo == "hello-world"
    assert action.pr_number == 42
    assert "Generated by GitHub Auto-Maintainer" in action.body
